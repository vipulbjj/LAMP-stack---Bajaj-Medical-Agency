...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["cpass"])) {
    $cpassErr = "Shop password is required";
  } else {
  ...
  if(($cpass != $_SESSION["pass"])||($idErr!="")){
    ...
    if($res3->num_rows < 1){
      $err = "No such equipment exists.";
    }
    else{
      $sql4 = "delete from centre".$cid." WHERE UId='$id'";
      if($conn->query($sql4)){
        $err = "Equipment deleted succesfully.";
      }
      else{
        $err = "Not able to delete equipment.";
      }
    }
  }
}
...
<h2 class="card-title">Delete Equipment</h2>
<!-- <p><span class="error">* required field</span></p>
 -->
 <form method="post" role ="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">

<div class="form-group" >
<label for="exampleInputName1" style="align:left">Equipment Id of the equipment being deleted</label>
...
<button type="submit" name="submit" class="btn btn-info">Delete Equipment</button><br><br>
<center>
<?php echo $err; ?>
</form>
</centre>
